...
from entropy_coder import HybridEntropyCoder, encode_image
...
class CCSDS123Compressor(nn.Module):
    ...
    - Closed-loop scalar quantization
    - Sample representative calculation  
    - Hybrid entropy coding
    """
    ...
    def __init__(
        ...
        # Entropy coder (initialized during compression)
    ...
    def _validate_input(self, image: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            image: Input multispectral/hyperspectral image
            
        Returns:
            Processed image tensor [Z, Y, X]
        """
        if image.dim() == 3:
            Z, Y, X = image.shape
    ...
    def forward(self, image: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Compress multispectral/hyperspectral image
    ...
    def _update_error_limits_adaptive(self, current_band: int, sample_count: int):
        """
    ...
    def compress(self, image: torch.Tensor) -> bytes:
        ...
        if self.compression_params['entropy_coder_type'] == 'hybrid':
            compressed_data = encode_image(results['mapped_indices'], self.num_bands)
        else:
            # Placeholder for other entropy coding methods
            compressed_data = b''
        
        return compressed_data
    
    def get_compression_stats(self, image: torch.Tensor) -> Dict[str, float]:
        """
...
def create_lossless_compressor(num_bands: int, **kwargs) -> CCSDS123Compressor:
    """
