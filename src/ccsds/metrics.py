...
def calculate_psnr(original, reconstructed, dynamic_range: int) -> float:
    """Peak signal-to-noise ratio in dB, with peak = 2^dynamic_range - 1.
...
def quality_report(original, reconstructed, dynamic_range: int) -> dict:
    """PSNR (dB), MSSIM, SAM (rad) and max absolute error, as a dict."""
    x, y = _as_float(original), _as_float(reconstructed)
    return {
        "psnr_db": calculate_psnr(x, y, dynamic_range),
