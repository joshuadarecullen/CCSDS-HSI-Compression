...
from ccsds.entropy_coder import HybridEntropyCoder, BlockAdaptiveEntropyCoder

# Try to import optimized components, fall back if not available
try:
    from optimized.optimized_compressor import OptimizedCCSDS123Compressor, create_optimized_lossless_compressor
    OPTIMIZED_AVAILABLE = True
except ImportError:
    OPTIMIZED_AVAILABLE = False
    print("Warning: Optimized components not available, testing standard components only")
...
def test_full_pipeline_verification(image):
    ...
    if OPTIMIZED_AVAILABLE:
        compressors = [
            ("Standard Lossless", lambda: create_optimized_lossless_compressor()),
            ("Optimized Lossless", lambda: OptimizedCCSDS123Compressor())
        ]
    else:
        print("Optimized compressors not available - testing manual pipeline only")
